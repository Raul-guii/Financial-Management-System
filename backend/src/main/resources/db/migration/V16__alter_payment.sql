CREATE TABLE audit_logs
(
    id          BIGINT AUTO_INCREMENT NOT NULL,
    created_at  datetime NULL,
    updated_at  datetime NULL,
    entity_id   BIGINT       NOT NULL,
    action      VARCHAR(254) NOT NULL,
    entity_type VARCHAR(100) NOT NULL,
    user_id     BIGINT       NOT NULL,
    CONSTRAINT pk_audit_logs PRIMARY KEY (id)
);

CREATE TABLE clients
(
    id                   BIGINT AUTO_INCREMENT NOT NULL,
    deleted_at           datetime NULL,
    created_at           datetime NULL,
    updated_at           datetime NULL,
    name                 VARCHAR(150) NOT NULL,
    type                 VARCHAR(20)  NOT NULL,
    document             VARCHAR(20)  NOT NULL,
    email                VARCHAR(150) NULL,
    phone                VARCHAR(20) NULL,
    address_street       VARCHAR(150) NULL,
    address_number       VARCHAR(20) NULL,
    address_neighborhood VARCHAR(100) NULL,
    address_city         VARCHAR(100) NULL,
    address_state        VARCHAR(50) NULL,
    address_postal_code  VARCHAR(20) NULL,
    address_country      VARCHAR(100) NULL,
    active               BIT(1) NULL,
    created_by           BIGINT NULL,
    CONSTRAINT pk_clients PRIMARY KEY (id)
);

CREATE TABLE contract_items
(
    id            BIGINT AUTO_INCREMENT NOT NULL,
    deleted_at    datetime NULL,
    created_at    datetime NULL,
    updated_at    datetime NULL,
    name          VARCHAR(100) NOT NULL,
    `description` VARCHAR(254) NULL,
    quantity      DECIMAL      NOT NULL,
    unit_price    DECIMAL      NOT NULL,
    active        BIT(1) NULL,
    contract_id   BIGINT       NOT NULL,
    CONSTRAINT pk_contract_items PRIMARY KEY (id)
);

CREATE TABLE contracts
(
    id             BIGINT AUTO_INCREMENT NOT NULL,
    deleted_at     datetime NULL,
    created_at     datetime NULL,
    updated_at     datetime NULL,
    status         VARCHAR(20) NOT NULL,
    billing_period VARCHAR(20) NOT NULL,
    start_date     date        NOT NULL,
    end_date       date        NOT NULL,
    created_by     BIGINT      NOT NULL,
    client_id      BIGINT      NOT NULL,
    CONSTRAINT pk_contracts PRIMARY KEY (id)
);

CREATE TABLE financial_parameters
(
    id            BIGINT AUTO_INCREMENT NOT NULL,
    deleted_at    datetime NULL,
    created_at    datetime NULL,
    updated_at    datetime NULL,
    type          VARCHAR(30)  NOT NULL,
    name          VARCHAR(100) NOT NULL,
    value         VARCHAR(254) NOT NULL,
    `description` VARCHAR(254) NOT NULL,
    active        BIT(1) NULL,
    updated_by    BIGINT       NOT NULL,
    created_by    BIGINT       NOT NULL,
    CONSTRAINT pk_financial_parameters PRIMARY KEY (id)
);

CREATE TABLE gateway_transactions
(
    id           BIGINT AUTO_INCREMENT NOT NULL,
    created_at   datetime NULL,
    updated_at   datetime NULL,
    status       VARCHAR(20)  NOT NULL,
    external_id  VARCHAR(254) NOT NULL,
    gateway_name VARCHAR(254) NOT NULL,
    amount       DECIMAL      NOT NULL,
    raw_response TEXT NULL,
    payment_id   BIGINT       NOT NULL,
    CONSTRAINT pk_gateway_transactions PRIMARY KEY (id)
);

CREATE TABLE invoice_lines
(
    id               BIGINT AUTO_INCREMENT NOT NULL,
    created_at       datetime NULL,
    updated_at       datetime NULL,
    `description`    VARCHAR(255) NOT NULL,
    quantity         DECIMAL      NOT NULL,
    unit_price       DECIMAL      NOT NULL,
    total            DECIMAL      NOT NULL,
    contract_item_id BIGINT NULL,
    invoice_id       BIGINT       NOT NULL,
    CONSTRAINT pk_invoice_lines PRIMARY KEY (id)
);

CREATE TABLE invoices
(
    id               BIGINT AUTO_INCREMENT NOT NULL,
    created_at       datetime NULL,
    updated_at       datetime NULL,
    status           VARCHAR(20) NOT NULL,
    issue_date       date        NOT NULL,
    due_day          date        NOT NULL,
    amount           DECIMAL     NOT NULL,
    late_free_amount DECIMAL     NOT NULL,
    interest_amount  DECIMAL     NOT NULL,
    contract_id      BIGINT NULL,
    CONSTRAINT pk_invoices PRIMARY KEY (id)
);

CREATE TABLE notifications
(
    id         BIGINT AUTO_INCREMENT NOT NULL,
    created_at datetime NULL,
    updated_at datetime NULL,
    title      VARCHAR(150) NOT NULL,
    message    VARCHAR(254) NOT NULL,
    type       VARCHAR(30)  NOT NULL,
    is_read    BIT(1) NULL,
    user_id    BIGINT       NOT NULL,
    CONSTRAINT pk_notifications PRIMARY KEY (id)
);

CREATE TABLE payments
(
    id             BIGINT AUTO_INCREMENT NOT NULL,
    created_at     datetime NULL,
    updated_at     datetime NULL,
    amount         DECIMAL      NOT NULL,
    payment_date   datetime     NOT NULL,
    payment_method VARCHAR(255) NOT NULL,
    invoice_id     BIGINT NULL,
    CONSTRAINT pk_payments PRIMARY KEY (id)
);

CREATE TABLE reconciliation_items
(
    id                     BIGINT AUTO_INCREMENT NOT NULL,
    system_amount          DECIMAL NOT NULL,
    gateway_amount         DECIMAL NOT NULL,
    reconciliation_id      BIGINT  NOT NULL,
    invoice_id             BIGINT  NOT NULL,
    payment_id             BIGINT  NOT NULL,
    gateway_transaction_id BIGINT  NOT NULL,
    CONSTRAINT pk_reconciliation_items PRIMARY KEY (id)
);

CREATE TABLE reconciliations
(
    id           BIGINT AUTO_INCREMENT NOT NULL,
    period_start date     NOT NULL,
    period_end   date     NOT NULL,
    executed_at  datetime NOT NULL,
    executed_by  BIGINT NULL,
    CONSTRAINT pk_reconciliations PRIMARY KEY (id)
);

CREATE TABLE refund_requests
(
    id           BIGINT AUTO_INCREMENT NOT NULL,
    status       VARCHAR(20)  NOT NULL,
    reason       VARCHAR(150) NOT NULL,
    requested_at datetime     NOT NULL,
    approved_at  datetime NULL,
    payment_id   BIGINT NULL,
    CONSTRAINT pk_refund_requests PRIMARY KEY (id)
);

CREATE TABLE users
(
    id         BIGINT AUTO_INCREMENT NOT NULL,
    deleted_at datetime NULL,
    created_at datetime NULL,
    updated_at datetime NULL,
    name       VARCHAR(100) NOT NULL,
    email      VARCHAR(150) NOT NULL,
    password   VARCHAR(254) NOT NULL,
    `role`     VARCHAR(50)  NOT NULL,
    CONSTRAINT pk_users PRIMARY KEY (id)
);

ALTER TABLE gateway_transactions
    ADD CONSTRAINT uc_gateway_transactions_external UNIQUE (external_id);

ALTER TABLE gateway_transactions
    ADD CONSTRAINT uc_gateway_transactions_payment UNIQUE (payment_id);

ALTER TABLE audit_logs
    ADD CONSTRAINT FK_AUDIT_LOGS_ON_USER FOREIGN KEY (user_id) REFERENCES users (id);

ALTER TABLE clients
    ADD CONSTRAINT FK_CLIENTS_ON_CREATED_BY FOREIGN KEY (created_by) REFERENCES users (id);

ALTER TABLE contracts
    ADD CONSTRAINT FK_CONTRACTS_ON_CLIENT FOREIGN KEY (client_id) REFERENCES clients (id);

ALTER TABLE contracts
    ADD CONSTRAINT FK_CONTRACTS_ON_CREATED_BY FOREIGN KEY (created_by) REFERENCES users (id);

ALTER TABLE contract_items
    ADD CONSTRAINT FK_CONTRACT_ITEMS_ON_CONTRACT FOREIGN KEY (contract_id) REFERENCES contracts (id);

ALTER TABLE financial_parameters
    ADD CONSTRAINT FK_FINANCIAL_PARAMETERS_ON_CREATED_BY FOREIGN KEY (created_by) REFERENCES users (id);

ALTER TABLE financial_parameters
    ADD CONSTRAINT FK_FINANCIAL_PARAMETERS_ON_UPDATED_BY FOREIGN KEY (updated_by) REFERENCES users (id);

ALTER TABLE gateway_transactions
    ADD CONSTRAINT FK_GATEWAY_TRANSACTIONS_ON_PAYMENT FOREIGN KEY (payment_id) REFERENCES payments (id);

ALTER TABLE invoices
    ADD CONSTRAINT FK_INVOICES_ON_CONTRACT FOREIGN KEY (contract_id) REFERENCES contracts (id);

ALTER TABLE invoice_lines
    ADD CONSTRAINT FK_INVOICE_LINES_ON_CONTRACT_ITEM FOREIGN KEY (contract_item_id) REFERENCES contract_items (id);

ALTER TABLE invoice_lines
    ADD CONSTRAINT FK_INVOICE_LINES_ON_INVOICE FOREIGN KEY (invoice_id) REFERENCES invoices (id);

ALTER TABLE notifications
    ADD CONSTRAINT FK_NOTIFICATIONS_ON_USER FOREIGN KEY (user_id) REFERENCES users (id);

ALTER TABLE payments
    ADD CONSTRAINT FK_PAYMENTS_ON_INVOICE FOREIGN KEY (invoice_id) REFERENCES invoices (id);

ALTER TABLE reconciliations
    ADD CONSTRAINT FK_RECONCILIATIONS_ON_EXECUTED_BY FOREIGN KEY (executed_by) REFERENCES users (id);

ALTER TABLE reconciliation_items
    ADD CONSTRAINT FK_RECONCILIATION_ITEMS_ON_GATEWAY_TRANSACTION FOREIGN KEY (gateway_transaction_id) REFERENCES gateway_transactions (id);

ALTER TABLE reconciliation_items
    ADD CONSTRAINT FK_RECONCILIATION_ITEMS_ON_INVOICE FOREIGN KEY (invoice_id) REFERENCES invoices (id);

ALTER TABLE reconciliation_items
    ADD CONSTRAINT FK_RECONCILIATION_ITEMS_ON_PAYMENT FOREIGN KEY (payment_id) REFERENCES payments (id);

ALTER TABLE reconciliation_items
    ADD CONSTRAINT FK_RECONCILIATION_ITEMS_ON_RECONCILIATION FOREIGN KEY (reconciliation_id) REFERENCES reconciliations (id);

ALTER TABLE refund_requests
    ADD CONSTRAINT FK_REFUND_REQUESTS_ON_PAYMENT FOREIGN KEY (payment_id) REFERENCES payments (id);